...
def main(args):
    ...
    # 建置預測用資料：混合 prefix 切法
    # 每場 rally 最多取幾個代表性 prefix：
    # 短 prefix：1, 2, 3
    # 中 prefix：5
    # 長 prefix：len(g) - 1，也就是最完整可用前綴
    ...
    for rid, g in train.groupby("rally_uid"):
        ...
        candidate_ends = [1, 2, 3, 5, len(g) - 1]

        candidate_ends = sorted(set([
            x for x in candidate_ends
            if 1 <= x < len(g)
        ]))

        for cut_end in candidate_ends:
            X = X_full[:cut_end]
