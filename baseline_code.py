...
def main(args):
    ...
    train = pd.read_csv(args.train).sort_values(["rally_uid","strikeNumber"])  #按照時間順序排好
    ...
    MAXLEN = max(L_list)
    X_all  = np.stack([pad2d(s, MAXLEN) for s in X_list])   #把資料長度補齊
    ...
    act_w = torch.tensor(1.0/act_counts, dtype=torch.float32); #出現次數越少 比重越大
    act_w = (act_w * (n_act/act_w.sum())) #把數字規模正常化
    ...
    #資料載入器
    train_loader = DataLoader(train_ds, batch_size=args.batch, shuffle=True) #把資料順序打亂=>可以真的學習
    ...
    # 2. 與 sample_submission 合併，確保所有 rally_uid 都在，並選取正確的欄位順序
    # 先將 sample 裡的預測欄位刪除，再合併我們預測出來的結果
    out = pd.read_csv(args.sample).drop(columns=["actionId","pointId","serverGetPoint"], errors="ignore")
